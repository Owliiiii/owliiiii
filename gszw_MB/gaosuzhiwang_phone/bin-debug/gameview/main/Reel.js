        ...
        if (this.isAction) {
            for (var i = 0; i < this.kGroup.numChildren; i++) {
                ...
                if (item.y >= this.height) {
                    var t1 = SetConst.SPEED_PLAY ? Reel.QUK_TIME : Reel.TIME;
                    //let t2: number = GameConfig.speedPlay ? Reel.QUK_STOPTIME : Reel.STOPTIME;
                    var t3 = SetConst.SPEED_PLAY ? [200, 350, 500, 650, 800] : [0, 400, 730, 930, 1030];
...
var ReelItem = (function (_super) {
    ...
    ReelItem.prototype.goAnimation = function () {
        ...
        egret.Tween.get(this.icons, { loop: true }).to({ scaleX: 0.8, scaleY: 0.8 }, 700).to({ scaleX: 1, scaleY: 1 }, 700);
    };
