...
var CheckBanner = (function (_super) {
        ...
        if (this.btnArr.length == 0) {
            for (var i = 0; i < 10; i++) {
                ...
                if (btn) {
                    ...
                    // this.btnArr[i].addEventListener(egret.TouchEvent.TOUCH_TAP, this.btnIndex, this);
                    // this.btnArr[i].name = 'btn' + i;
    ...
    CheckBanner.prototype.btnIndex = function (evt) {
        var index = evt.currentTarget.name.substr(3);
        console.log("index == " + index);
        for (var k = 0; k < 5; k++) {
            if (k == parseInt(index)) {
                this.btnArr[k].selected = true;
            }
            else {
                this.btnArr[k].selected = false;
            }
        }
    };
